...
/*
// обычный синтаксис
$name = 'Игорь'; // одинарные кавычки
echo "Меня зовут $name"; // двойные, интерпретируют $name
*/

/*
// синтаксис heredoc (двойные кавычки можно не указывать)
echo <<<"FOOBAR"
Привет, $name!
FOOBAR;

// nowdoc, $name не интерпретируется
echo <<<'FOOBAR'
Привет,$name!
FOOBAR;
*/
...
/*
Object - Объект - это экземпляр, частное состояние Класса. 
Создается из Класса при помощи использования выражения new.
*/
...
var_dump($tree); // object(Tree)#1 (0) {...}
print_r( gettype($tree) ); // object
*/
...
Оператор instanceof также может быть использован для определения,
наследует ли определенный объект какой-либо класс 
или реализует ли класс заданный интерфейс.
*/
...
соединение с базой данных, ссылка на открытый файл, открытое http-соединение и др.

Примеры типов ресурса:
curl, ftp, gd, mysql link, mysql result, stream, socket
*/

/*
is_resource(); // является ли переменная ресурсом
get_resource_type(); // получить тип данного ресурса
*/
...
/*
Iterable - псевдотип обозначающий итерируемый тип данных. 
Относится к смешанным типам, на данный момент может использоваться для уточнения типа array или object, 
реализующего интерфейс Traversable.
*/
...
var_dump( a([]) ); // array
var_dump( a(123) ); // Fatal error: Uncaught TypeError: Argument 1 passed to a() must be iterable, integer given
*/
...
/*
Callable - псевдотип, используется для уточнения передаваемого аргумента 
(функции или метода объекта), которые можно вызывать. 
...
/*
// settype(); - постоянное приведение типа
...
$a * $b; // Умножение
$a / $b; // Деление
$a % $b; // Деление по модулю (возвращает целочисленный остаток от деления)
...
$a <> $b; // (тоже самое) $a не равно $b после преобразования типов.
...
$a <=> $b;
// Спейсшип. Этот оператор предназначен для сравнения двух выражений (с приведением типов). 
...
Работает только с выражениями.
Рекомендуется добавлять в код соответствующие проверки, и не использовать оператор подавления ошибок.
*/
...
/*
синтаксис (полная форма):
(expr1) ? (expr2) : (expr3)
Если expr1 - true, вернет expr2. Если expr1 - false, вернет expr3.

синтаксис (сокращенная форма):
(expr1) ? : (expr3)
Если expr1 - true, вернет true. Если expr1 - false, вернет expr3.
*/
...
Оператор объединения с null (условный оператор). 
Он возвращает первый операнд, если он задан и не равен null, а в обратном случае возвращает второй операнд.
*/

// если ячейка не определена, в переменную запишется Аноним (задаем значение по-умолчанию).
...
// массив массивов, многомерный массив (в данном случает двумерный - уровень вложенности 2)
$arrArrs = [[1,2,3], [4,5,6], [7,8,9]];

// добавление значений в индексированный массив,
// если индекс не указывать, он высчитывается последовательно начиная с 0
$fruit[] = "apple";
$fruit[] = "pear";
$fruit[] = "plum";
...
// если же мы хотим вернуть имя ключа - переменная $key обязательна
// здесь $key и $value - 2 независимые переменные
foreach ($arrNums as $key => $value) {
  echo "<p>source: $arrNums, key: $key, value: $value</p>";
}

// чтобы значение ячейки перезаписать, нам нужно знать имя ключа, поэтому переменная $key обязательна
// здесь $key и $value - 2 независимые переменные
foreach ($arrNums as $key => $value) {
  $arrNums[$key] = pow($value, 2);
}

// &$value - & перед именем переменной создает передачу значения в нее по ссылке
// т.е. $value становится ссылочной переменной, которая ссылается на родительскую ячейку памяти переменной $key
foreach ($arrNums as $key => &$value) {
  // &$value -> $arrNums[$key]
  $value = pow($value, 2);
}

// &$value в таком виде становится связанной с ключем (с родительской ячейкой памяти),
// поэтому меняя значение переменной $value, мы меняем значение родительской ячейки памяти
foreach ($arrNums as &$value) {
  $value = pow($value, 2);
}
...
// теперь обе переменные - ссылочные т.к. они ссылаются на одну область памяти,
// изменение значения в одной переменной меняет значение в другой
var_dump($b); // $b === 5
$b = 25;
var_dump($a); // $a === 5 -> $a === 25
$a = null;
...
$a <> $b; // (тоже самое) Не равно - TRUE, если массив $a не равен массиву $b.
